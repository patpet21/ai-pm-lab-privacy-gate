(() => {
  ...
  function notice(message, kind = "normal") {
    ...
    clearTimeout(window.__privacyGateUnifiedFileNoticeTimerV3);
    window.__privacyGateUnifiedFileNoticeTimerV3 = setTimeout(() => node.remove(), 5200);
  }
  ...
  function inputScore(input) {
    ...
    if (/file|upload|attach/.test(name)) score += 25;
    return score;
  }
  ...
  function attachTrigger() {
    const box = composer();
    const scope = box?.closest("form") || box?.closest("fieldset") || box?.closest("rich-textarea")?.parentElement?.parentElement || box?.parentElement?.parentElement || document;
    const hostSelectors = host === "chatgpt.com"
      ? ['button[data-testid*="composer-plus" i]', 'button[aria-label*="attach" i]', 'button[aria-label*="add" i]']
      : host === "claude.ai"
        ? ['button[aria-label*="attach" i]', 'button[aria-label*="add" i]', 'button[data-testid*="attach" i]']
        : ['button[aria-label*="upload" i]', 'button[aria-label*="attach" i]', 'button[aria-label*="add" i]'];
    const common = [
      'button[data-testid*="upload" i]', 'button[data-test-id*="upload" i]',
      '[role="button"][aria-label*="attach" i]', '[role="button"][aria-label*="upload" i]', '[role="button"][aria-label*="add" i]'
    ];
    for (const selector of [...hostSelectors, ...common]) {
  ...
  async function attachThroughNativeInput(file, preferredInput = null) {
    const preferred = bestInput(preferredInput);
    if (preferred && injectProtected(preferred, file)) {
      await sleep(300);
      return true;
    }
  ...
  function errorMessage(response) {
    ...
    if (code === "file_worker_unavailable") return `PrivacyGate — isolated file worker failed${detail ? `: ${detail}` : ""}. The original file was not attached.`;
  ...
  async function handoffProtectedFile(file, preferredInput) {
    if (await attachThroughNativeInput(file, preferredInput)) {
  ...
  async function protect(file, preferredInput = null) {
    ...
    try {
      ...
      const handedOff = await handoffProtectedFile(protectedFile, preferredInput);
      if (handedOff) {
        notice(`PrivacyGate — ${filename} protected locally and attached.`, "success");
        return;
      }

      notice(`PrivacyGate — ${filename} is protected, but ${providerName()} did not expose a usable file input. The original file was not sent.`, "error");
    } catch (error) {
      setBusy(false);
