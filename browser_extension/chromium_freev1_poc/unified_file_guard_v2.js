(() => {
  ...
  function protectedMime(name, fallback = "") {
    return ({
      ".pdf": "application/pdf",
      ...
      ".jpg": "image/jpeg",
      ".jpeg": "image/jpeg"
    })[suffixOf(name)] || fallback || "application/octet-stream";
  }
  ...
  function shieldNativeDragUi(event) {
    if (!enabled || internalInjecting || !event.isTrusted || !carriesFiles(event)) return;
    stop(event);
    try { event.dataTransfer.dropEffect = "copy"; } catch (_error) {}
  }

  document.addEventListener("dragenter", shieldNativeDragUi, true);
  document.addEventListener("dragover", shieldNativeDragUi, true);
  ...
  function notice(message, kind = "normal") {
    ...
    if (!node) {
      ...
      Object.assign(node.style, {
        ...
        zIndex: "2147483647", maxWidth: "min(660px,calc(100vw - 32px))", padding: "12px 16px",
    ...
    clearTimeout(window.__privacyGateUnifiedFileNoticeTimerV2);
    window.__privacyGateUnifiedFileNoticeTimerV2 = setTimeout(() => node.remove(), 4600);
  }
  ...
  function setBusy(value, label = "PrivacyGate processing file locally…") {
    ...
    Object.assign(pill.style, {
      ...
      border: "1px solid #d8e1ec", boxShadow: "0 14px 40px rgba(15,23,42,.24)", fontSize: "13px",
      fontWeight: "750"
    });
  ...
  function base64ToFile(encoded, filename, original) {
    ...
    return new File([bytes], filename, {
      type: protectedMime(filename, original?.type),
  ...
  function bestInput(preferred = null) {
    ...
    for (const input of nativeInputs()) {
      ...
      let score = 0;
      if (!accept || /pdf|docx|word|sheet|excel|presentation|powerpoint|image|text|xlsx|pptx/.test(accept)) score += 100;
  ...
  function pageShowsFile(file) {
    ...
    return stem.length >= 12 && text.includes(stem.slice(0, Math.min(stem.length, 24)));
  }
  ...
  function attachTrigger() {
    ...
    for (const selector of selectors) {
      const found = Array.from(scope.querySelectorAll?.(selector) || []).find(el => el instanceof HTMLElement && !el.closest('[id^="privacygate-"]'));
      if (found) return found;
    }
    return Array.from(scope.querySelectorAll?.('button,[role="button"]') || []).find(el => {
      if (!(el instanceof HTMLElement) || el.closest('[id^="privacygate-"]')) return false;
  ...
  function errorMessage(response) {
    ...
    if (code === "file_worker_unavailable") return "PrivacyGate — the isolated file worker failed. The desktop stayed protected and the original file was not attached.";
  ...
  async function handoffProtectedFile(file, preferredInput, dropTarget) {
    if (preferredInput instanceof HTMLInputElement && preferredInput.isConnected && injectProtected(preferredInput, file)) {
  ...
  async function protect(file, preferredInput = null, dropTarget = null) {
    ...
    try {
      ...
      const handedOff = await handoffProtectedFile(protectedFile, preferredInput, dropTarget);
      if (handedOff) {
      ...
      notice(
        `PrivacyGate — ${filename} is protected, but ${providerName()} did not expose a usable attachment target. The original file was not sent.`,
        "error"
      );
    } catch (error) {
      setBusy(false);
  ...
  document.addEventListener("change", event => {
    ...
    if (internalInjecting || !event.isTrusted || !enabled || busy) return;
  ...
  document.addEventListener("drop", event => {
    ...
    if (!files.length) return;
    const dropTarget = event.target instanceof Element ? event.target : composer();
    stop(event);
    if (files.length !== 1) {
      notice("PrivacyGate — drop one file at a time. Nothing was attached.", "error");
      return;
    }
    protect(files[0], null, dropTarget);
  }, true);
  ...
  document.addEventListener("paste", event => {
    ...
    protect(files[0], bestInput(), null);
  }, true);
