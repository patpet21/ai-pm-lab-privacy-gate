(() => {
  ...
  const BAR_ID = "privacygate-multi-ai-bar";
  const REVIEW_ID = "privacygate-multi-ai-review";
  let enabled = true;
  let busy = false;
  ...
  function sendButton() {
    const box = composer();
    if (!box) return null;
    const scope = box.closest("form") || box.closest("fieldset") || box.parentElement?.parentElement || document;
    const selectors = [
  ...
  function notice(message, kind = "normal") {
    ...
    if (!element) {
      ...
      style(element, {
        ...
        padding: "11px 15px", borderRadius: "10px", color: "#fff",
        fontFamily: "Arial, sans-serif", fontSize: "13px", fontWeight: "750",
  ...
  function updateBar() {
    ...
    const stateText = enabled ? "Protection ON" : "Protection OFF";
    ...
    if (state) state.style.color = enabled ? "#86EFAC" : "#CBD5E1";
  ...
  function ensureBar() {
    ...
    const bar = style(document.createElement("div"), {
      ...
      zIndex: "2147483645", display: "flex", alignItems: "center", gap: "8px",
      padding: "7px 10px", borderRadius: "999px", background: "rgba(15,23,42,.94)",
      color: "#fff", border: "1px solid rgba(148,163,184,.28)",
      boxShadow: "0 8px 28px rgba(15,23,42,.24)", fontFamily: "Arial, sans-serif",
      fontSize: "11px", fontWeight: "750", backdropFilter: "blur(8px)"
    });
    bar.id = BAR_ID;

    const mark = style(document.createElement("span"), {
      width: "18px", height: "18px", display: "grid", placeItems: "center",
      borderRadius: "50%", background: "#2348B5", fontSize: "10px", fontWeight: "900"
    });
    mark.textContent = "PG";
    ...
    const toggle = style(document.createElement("button"), {
      width: "38px", height: "22px", padding: "2px", margin: "0 0 0 2px",
  ...
  function setComposerText(box, text) {
    ...
    let inserted = false;
    try { inserted = document.execCommand("insertText", false, text); } catch (_error) { inserted = false; }
  ...
  function sendApproved(text) {
    ...
    const trySend = attempt => {
      ...
      if (attempt < 20) return setTimeout(() => trySend(attempt + 1), 75);
  ...
  function showReview(textSnapshot, findings) {
    ...
    const overlay = style(document.createElement("div"), {
      ...
      background: "rgba(15,23,42,.42)", backdropFilter: "blur(2px)", fontFamily: "Arial, sans-serif"
    });
    overlay.id = REVIEW_ID;
    const card = style(document.createElement("section"), {
      width: "min(620px, 94vw)", maxHeight: "min(720px, 88vh)", display: "flex",
      flexDirection: "column", overflow: "hidden", background: "#fff", color: "#172033",
      border: "1px solid #DDE3EA", borderRadius: "18px", boxShadow: "0 24px 70px rgba(15,23,42,.28)"
    });
    ...
    protect.addEventListener("click", () => {
      const ids = Array.from(list.querySelectorAll('input[type="checkbox"]:checked'))
        .map(item => item.dataset.findingId).filter(Boolean);
  ...
  function analyzeAndGate() {
    ...
    chrome.runtime.sendMessage({ type: "PG_ANALYZE", text: snapshot }, response => {
      ...
      if (!findings.length) return sendApproved(snapshot);
  ...
  window.addEventListener("pagehide", () => window.clearInterval(barTimer), { once: true });
