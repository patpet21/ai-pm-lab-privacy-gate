...
def _make_help_card() -> QFrame:
    ...
    for number, heading, body in steps:
        ...
        text = QLabel(
            f"<b>{heading}</b><br><span style='color:#66788a'>{body}</span>"
        )
...
def _apply_redesign(page: ProtectionPage) -> None:
    ...
    page.local_badge.setText(
        "100% LOCAL  |  NO CLOUD UPLOAD  |  RESTORE AVAILABLE"
    )
    ...
    upload_zone_layout.addWidget(upload_label)
    ...
    scan_button.setStyleSheet(
        ...
        "border-radius:8px;font-size:15px;font-weight:700;padding:10px 24px;}"
    ...
    confidence.setRange(
        page.threshold_input.minimum(), page.threshold_input.maximum()
    )
    ...
    for text, widget in (
        ...
        field = QLabel(text)
        field.setStyleSheet(
            "font-size:11px;color:#6c7f90;font-weight:600;"
        )
    ...
    advanced_toggle.toggled.connect(
        lambda checked: advanced_toggle.setText(
            "v  Advanced protection settings"
            if checked
    ...
    wrapper_layout.addWidget(_make_help_card())
    ...
    def sync_file(path: str) -> None:
        if path:
            ...
            upload_filename.setText("Choose a local file")
        invalidate_result()
    ...
    def save_only() -> None:
        ...
        if saved:
            QMessageBox.information(
                page,
                "Saved locally",
                "The protected document and its reversible mapping "
                "were saved to your local library.",
            )
...
def install_redesign() -> None:
    ...
    def redesigned_init(
        self: ProtectionPage, *args, **kwargs
    ) -> None:
    ...
    def redesigned_analysis_ready(
        ...
        self.findings_metric.setText(
            f"{len(self.current_findings)} detected  |  0 protected"
        )
        self._last_residual = ()
        self.verification_metric.setText(
            "Ready to protect selected items"
        )
    ...
    def redesigned_set_busy(
        self: ProtectionPage, busy: bool
    ) -> None:
